...
export default function WalletPage() {
  ...
  const loadWallet = async () => {
    ...
    const { data: txs } = await supabase
      ...
      .select(
        "*, from_user:profiles!from_user(username), to_user:profiles!to_user(username)"
      )
  ...
  // ✅ Realtime listener MUST be inside the component
  useEffect(() => {
    const sub = supabase
      .channel("realtime:wallets")
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "flip_wallets" },
        (payload) => {
